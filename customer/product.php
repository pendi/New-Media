					...
					<ul class="pagination">	
						...
						<?php if($jmlhalaman != 0): ?>
							<?php if($halaman > 1): ?>
								...
								<li><a href="<?php echo "$_SERVER[PHP_SELF]?halaman=$previous&id=$idc&by=$by" ?>" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
							<?php else: ?>
						...
						<?php for($i=1;$i<=$jmlhalaman;$i++): ?>
							<?php if($i>=($halaman-3) && $i <= ($halaman+3)): ?>
								<?php if ($i != $halaman): ?>
									<li><a href="<?php echo "$_SERVER[PHP_SELF]?halaman=$i&id=$idc&by=$by" ?>"><?php echo $i; ?></a></li>
								<?php else: ?>
						...
						<?php if($jmlhalaman != 0): ?>
							<?php if($halaman < $jmlhalaman): ?>
								<?php $next = $halaman+1; ?>
								<li><a href="<?php echo "$_SERVER[PHP_SELF]?halaman=$next&id=$idc&by=$by" ?>" aria-label="Next"><span aria-hidden="true">»</span></a></li>
							<?php else: ?>
